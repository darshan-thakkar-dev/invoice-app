...
fclose($file);
	...
	include("includes/config.php");
    	...
    	$query_teacher=$con->select_query("tbl_cust","*","","");
            if($con->total_records($query_teacher) != 0)
            {
                ...
                while($row_teacher = mysqli_fetch_array($query_teacher))
                {
                    ...
                    echo json_encode(array("status"=>1,"details"=>$TeacherList,"message"=>"Customer Listed Successfully."));
                }else{
                    header('Content-type: application/json');
                    echo json_encode(array("status"=>1,"details"=>array(),"message"=>"Customer Listed Successfully."));
                }
?>