	...
	include("includes/config.php");	
    	...
    	if (empty($company_name) || empty($id)) {
			...
			if(mysqli_num_rows($query_class_duplicate) < 0)
			{
                ...
                $qry="SELECT * FROM `tbl_sup` WHERE `id`=".$id;
    		...
    		$insertUser = $con->update("tbl_sup",$state,"where id='".$id."'");
					// $user_id = mysqli_insert_id();
					//print_r($userid);die();
					header('Content-type: application/json');
					echo json_encode(array("status"=>1,"message"=>"Supplier update Successfully."));
    	}
    }
?>